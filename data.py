...
data_stats = {'MNIST': ((0.1307,), (0.3081,)),
...
def fetch_dataset(data_name, conf=None):
    import datasets
    dataset = {}
    ...
    if data_name in ['MNIST', 'FashionMNIST']:
        ...
        dataset['test'].transform = datasets.Compose([
            transforms.ToTensor(),
            transforms.Normalize(*data_stats[data_name])])
    elif data_name in ['BDD100K']:
        train_transform = TrainAugmentation(conf["image_size"], conf["image_mean"], conf["image_std"]) if conf else None
        target_transform = MatchPrior(conf["priors"], conf["center_variance"], conf["size_variance"], 0.5) if conf else None
        dataset['train'] = VOCDataset('../bdd100k/images/100k/train/', transform=train_transform, target_transform=target_transform)
        d_num = int(len(dataset['train'].ids)*0.8)
        dataset['test'] = copy.deepcopy(dataset['train'])
        dataset['train'].ids, dataset['test'].ids = dataset['train'].ids[:d_num], dataset['test'].ids[d_num:]
    else:
...
def separate_dataset(dataset, idx):
    ...
    if cfg['data_name'] in ['BDD100K']:
        separated_dataset.ids = [separated_dataset.ids[s] for s in idx]
    else:
...
def separate_dataset_su(server_dataset, client_dataset=None, supervised_idx=None):
    if supervised_idx is None:
        ...
        elif cfg['data_name'] in ['BDD100K']:
            if cfg['num_supervised'] == -1:
                raise
            else:
                idx = torch.randperm(len(server_dataset.ids)).tolist()
                supervised_idx = idx[:cfg['num_supervised']]
                # unsupervised_idx = idx[cfg['num_supervised']:] ####### Modify ####### uncomment
                n_clnt_data = 64
                unsupervised_idx = idx[cfg['num_supervised']:cfg['num_supervised']+n_clnt_data*cfg['num_clients']] ####### Modify ####### Remove
...
class MixDataset(Dataset):
    ...
    def __getitem__(self, index):
        # if cfg['data_name'] in ["BDD100K"]:
        #     input = [self.dataset.data[index], self.dataset.box[index], self.dataset.target[index]]
        # else:

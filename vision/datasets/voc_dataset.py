...
class VOCDataset:
	...
	def __getitem__(self, index):
		...
		############################################################
		boxes = torch.nan_to_num(boxes, posinf=84, neginf=-84)
		# if (boxes.min()<-100 or boxes.max()>100):
		# 	print(boxes.min(), boxes.max())
		# image = self._read_image(image_id)
		# bb, ll, ii = self._get_annotation(image_id)
		# image1, bb1, ll1 = self.transform(image, bb, ll)
		# bb2, ll2 = self.target_transform(bb1, ll1)
		############################################################
	...
	def _get_annotation(self, image_id):
		...
		for object in objects:
			...
			if class_name in self.class_dict:
				...
				slc = 0
				# slc = 1
				x1 = float(bbox.find('xmin').text) - slc
				y1 = float(bbox.find('ymin').text) - slc
				x2 = float(bbox.find('xmax').text) - slc
				y2 = float(bbox.find('ymax').text) - slc

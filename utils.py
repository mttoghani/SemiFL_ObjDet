...
def process_dataset(dataset, ObjDet=False):
    cfg['data_size'] = {'train': len(dataset['train']), 'test': len(dataset['train'])}
    cfg['target_size'] = len(dataset['train'].class_names) if ObjDet else dataset['train'].target_size
    return
...
def process_control():
    ...
    data_shape = {'MNIST': [1, 28, 28], 'FashionMNIST': [1, 28, 28], 'CIFAR10': [3, 32, 32], 'CIFAR100': [3, 32, 32],
                  'SVHN': [3, 32, 32], "BDD100K": [3, 300, 300]}
    ...
    if 'num_clients' in cfg['control']:
        ...
        if cfg['num_supervised'] > 1000:
            cfg['server']['batch_size'] = {'train': 64, 'test': 64}
        else:
            cfg['server']['batch_size'] = {'train': 32, 'test': 64}
        # cfg['server']['num_epochs'] = int(np.ceil(float(cfg['local_epoch'][1])))
        cfg['server']['num_epochs'] = int(cfg['control']['local_epoch'])
        cfg['client'] = {}
        ...
        cfg['server']['num_epochs'] = int(cfg['control']['local_epoch'])
        cfg['client']['num_epochs'] = int(cfg['control']['local_epoch'])
        cfg['local'] = {}
